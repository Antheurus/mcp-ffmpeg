...
server.tool("get-ffmpeg-version", "Get the version of FFmpeg installed on the system", {}, async () => {
    try {
...
server.tool("resize-video", "Resize a video to one or more standard resolutions", {
...
server.tool("extract-audio", "Extract audio from a video file", {
...
server.tool("get-video-info", "Get detailed information about a video file", {
    ...
    try {
        ...
        if (videoInfo.streams && videoInfo.streams.length > 0) {
            ...
            videoInfo.streams.forEach((stream, index) => {
